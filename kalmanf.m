...
if isnan(s.x)
   ...
   tempupdx = s.x + K*(s.z-s.H*s.x);
   tempupdP = s.P - K*s.H*s.P;
   if abs(resi(1,:))<thred1
       s.x(1,:) = tempupdx(1,:);
       s.P(1,:) = tempupdP(1,:);
   end
   if abs(resi(2,:))<thred2
       s.x(2,:) = tempupdx(2,:);
       s.P(2,:) = tempupdP(2,:);
   end
   if abs(resi(3,:))<thred3
       s.x(3,:) = tempupdx(3,:);
       s.P(3,:) = tempupdP(3,:);
   end
   if abs(resi(4,:))<thred4
       s.x(4,:) = tempupdx(4,:);
       s.P(4,:) = tempupdP(4,:);
   end
